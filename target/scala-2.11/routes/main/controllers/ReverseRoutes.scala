    ...
    def vote(tuiId:String, sender:Long, eventId:Long, folio:Long): Call = {
      ...
      (tuiId: @unchecked, sender: @unchecked, eventId: @unchecked, folio: @unchecked) match {
        ...
        case (tuiId, sender, eventId, folio) if folio == -1 =>
          implicit val _rrc = new ReverseRouteContext(Map(("folio", -1)))
          Call("GET", _prefix + { _defaultPrefix } + "vote/" + implicitly[PathBindable[Long]].unbind("eventId", eventId) + "/" + implicitly[PathBindable[Long]].unbind("sender", sender) + "/" + implicitly[PathBindable[String]].unbind("tuiId", dynamicString(tuiId)) + "/")
